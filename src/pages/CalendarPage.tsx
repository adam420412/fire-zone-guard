...
export default function CalendarPage() {
  ...
  const getTasksForDay = (day: Date) => {
    return allTasks.filter(t => t.deadline && isSameDay(new Date(t.deadline), day));
  };
              ...
              return (
                ...
                >
                  ...
                  <div className="space-y-0.5">
                    {dayTasksCount.slice(0, 2).map(task => (
                      <div
                        key={task.id}
                    ...
                    {dayTasksCount.length > 2 && (
                      <div className="text-[9px] text-muted-foreground font-medium pl-1">+{dayTasksCount.length - 2} więcej</div>
                    )}
                  </div>
                </div>
              );
              ...
              >
                <div className="flex items-start gap-3">
                  <div className={cn("mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full",
                    task.status === "Zamknięte" ? "bg-success/20" : task.isOverdue ? "bg-critical/20" : "bg-primary/20"
                  )}>
