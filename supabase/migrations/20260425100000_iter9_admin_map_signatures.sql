-- =============================================================================
-- Iter 9 — Admin Edit Everywhere + Map + E-signatures + Cost tracking
--
-- 1. buildings: lat/lng (cache geocodingu Nominatim) + display_color (override
--    pin koloru na mapie).
--
-- 2. service_protocols: inspector_signature_url, client_signature_url,
--    inspector_signed_at, client_signed_at, client_signer_name.
--
-- 3. tasks: cost_actual NUMERIC (rzeczywisty koszt naprawy), cost_currency
--    TEXT DEFAULT 'PLN'. cost_estimate juz istnieje przy SLA tickets.
--
-- 4. sla_tickets: cost_estimate (jesli nie ma) — fallback dla MTBF analytics.
-- =============================================================================
...
-- RLS: signed-in users can upload, anyone can read (signatures are not secret
-- and the URLs are not enumerable — they are randomized UUIDs)
...
-- ---- 4. View: building_cost_summary (per-building koszty napraw 12mc) -------
...
COMMENT ON VIEW public.building_cost_summary IS 'Agregat kosztow napraw per obiekt za ostatnie 12 miesiecy. Uzywane w AnalyticsPage tab "Koszty".';
